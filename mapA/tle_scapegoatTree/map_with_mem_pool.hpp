...
private:
    ...
    struct ramPool {
        Node* root;
        ramPool(): root(nullptr) {}
        ~ramPool() {
            Node* x;
            while(root) {
                x = root, root = root->nxt;
                delete x;
            }
        }
        Node* newNode(value_type* const v) {
            Node* ret = root;
            if(root) root = root->nxt;
            else ret = new Node;
            ret->ls = ret->rs = ret->fa = ret->nxt = nullptr;
            ret->siz = 1, ret->v = v;
            return ret;
        }
        void deleteNode(Node* const x) {
            x->nxt = root, root = x;
            delete x->v; x->v = nullptr;
        }
    }mp;
        ...
        while(1) {
            ...
            if(cmp(cur->v, v)) {
                if(cur->rs) cur = cur->rs;
                    ...
                    cur->rs = mp.newNode(v), cur->rs->fa = cur;
                ...
                if(cur->ls) cur = cur->ls;
                    ...
                    cur->ls = mp.newNode(v), cur->ls->fa = cur;
...
public:
	...
	void clear() { deleteAll(root), root = mp.newNode(nullptr); }
